  ...
  useEffect(() => {
    const fetchUploadedFiles = () => {
      ...
      const userFiles = files.filter(
        (file) => file.walletAddress === account?.address
      );
        ...
        {!uploadedFiles.length ? (
          ...
          <div className="flex flex-wrap gap-4 text-white">
            {uploadedFiles.map((file, index) => (
              <div
                key={index}
                className="relative text-sm whitespace-nowrap mb-2 sm:mb-0 bg-transparent rounded-lg backdrop-blur-lg p-4 shadow-lg w-full max-w-[366px]">
                {isImageFile(file.fileName) ? (
                  // Load the image from IPFS using the IPFS hash
                  <div className="h-[153px] overflow-hidden rounded w-full">
                    <img
                ...
                <p
                  className="w-full px-4 py-2 text-[#0094FF] rounded-[2em] border-slate-800 shadow-lg
                  transform hover:scale-105 transition-transform duration-300 border-gradient2 bg-opacity-50
                  backdrop-filter backdrop-blur-lg flex items-center text-left relative text-xs
                  overflow-hidden text-ellipsis whitespace-nowrap max-w-fit mt-4">
                  {file.fileName}
                </p>
                ...
                <button
                  ...
                  className="inline-block mt-5 bg-[#0094FF] text-sm text-white py-4 font-medium px-10 rounded-[2em] mb-5">
                  {isVideoFile(file.fileName)
                    ? "Download Video"
                    : "Download Image"}
                </button>
