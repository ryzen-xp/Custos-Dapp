...
const Recorder = () => {
  ...
  const startRecording = (thisButton, otherButton) => {
    ...
    navigator.mediaDevices.getUserMedia(constraints)
      .then((mediaStream) => {
        ...
        document.getElementById('vid-record-status').innerText = 'Click the "Stop" button to stop recording';
  ...
  const stopRecording = async () => {
    if (mediaRecorder.state === "recording") {
      ...
      document.getElementById('vid-record-status').innerText = 'Click the "Start" button to start recording';
    ...
    <div>
      ...
      <div className="flex flex-col items-center justify-center h-screen border-2 mx-auto border-[#baa] my-10 bg-[#090909]">
        <div className="display-none" id="vid-recorder">
          ...
          <button type="button" id="start-vid-recording" onClick={(e) => startRecording(e.target, document.getElementById('stop-vid-recording'))} className="bg-[#c92eff] w-fit rounded-lg hover:bg-[#090909] text-white font-bold py-2 px-4 border-2 border-[#c92eff] font-san hover:border-[#c92eff]">
            Start
          </button>

          <button type="button" id="take-picture" onClick={takePicture} className="bg-[#c92eff] w-fit rounded-lg hover:bg-[#090909] text-white mx-3 font-bold py-2 px-4 border-2 border-[#c92eff] font-san hover:border-[#c92eff]">
            Take Picture
          </button>

          <button type="button" id="stop-vid-recording" onClick={stopRecording} className="bg-[#c92eff] w-fit  rounded-lg hover:bg-[#090909] text-white font-bold py-2 px-4 border-2 border-[#c92eff] font-san hover:border-[#c92eff]">
            Stop
          </button>
          <a id="download-video" style={{ display: 'none' }} href={videoUrl} download="recorded-video.webm">Download Video</a>
          <a id="download-image" style={{ display: 'none' }} href={imageUrl} download="captured-image.png">Download Image</a>
        </div>
      </div>
    </div>
  );
