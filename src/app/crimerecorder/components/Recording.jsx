...
export const Recording = ({ text, icon1, imgText, category }) => {
  ...
  const startRecording = async () => {
    ...
    if (!mediaStream) {
      ...
      openNotification("error", "", `Media stream not available`);
      return;
    }
  ...
  async function uploadToIPFS(fileBlob, fileName) {
    ...
    try {
      ...
      if (!response.ok) {
        openNotification("error", "", "Failed to upload file to IPFS");
      ...
      openNotification(
        "success",
        "",
        "File uploaded succesfully and data saved!"
      );
      setShowModal(true); // Open the success modal after upload
    } catch (error) {
      console.error("Error uploading file:", error);
      openNotification("error", "Error uploading file", `${error}`);
    }
  }
  ...
  const handleStopMedia = async () => {
    ...
    if (!account) {
      console.error("Account not connected");
          openNotification("error", "", "Account not connected");
      return;
    }
  };
