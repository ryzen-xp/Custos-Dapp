...
const AgreementCard = ({ agreement }) => {
    return (
      <div className="p-3 text-base space-y-[1em] flex flex-col bg-gradient-to-r border-gradient h-fit backdrop-blur-2xl from-[#19B1D2] to-[#0094FF] bg-clip-text text-transparent rounded-lg relative">
        ...
        <p className="br w-fit border-slate-800 px-2 py-[0.8] border text-[0.7em]  text-[#9B9292]">Second Party Address: {agreement[2]}</p>
        <div className='w-full flex items-center justify-start space-x-14 text-[1.2em] text-white'>Created by: <span className="text-[1em] font-bold bg-gradient-to-r from-[#19B1D2] to-[#0094FF] bg-clip-text text-transparent">{agreement[3]}</span></div>
