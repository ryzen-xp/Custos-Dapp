...
const ServiceDetails = ({ params }) => {
  ...
  const makeBoldBeforeColon = (text) => {
    ...
    if (splitText.length > 1) {
      ...
      return (
        <p>
          <strong className="text-[#c8a2c8] text-[1.2em]">
            {splitText[0]}:
          </strong>{" "}
          {splitText.slice(1).join(":")}
        </p>
      ); // Wrap first part before colon in <strong> tag
    }
    return <p>{text}</p>;
  };
    ...
    <div className="w-full">
      ...
      <div className="border-[#c8a2c8] w-[80%] m-auto flex flex-col rounded-lg my-4 items-center justify-center border text-white p-6">
        ...
        <Image
          ...
          width={100}
          height={100}
          className="w-full h-[39vh] rounded-lg mb-6 animate-fadeIn"
        />
        {process.content.map((content, index) => (
          <p
            key={index}
            className="text-lg mb-2 p-4 border rounded-lg border-[#c8a2c8] bg-[#110414] animate-fadeIn"
          >
            {makeBoldBeforeColon(content)}
          </p>
        ))}
        <Link
          href={`/${decodeURIComponent(title)
            .replaceAll(" ", "")
            .toLowerCase()}`}
        >
